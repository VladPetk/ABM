...
function PChart({ title, sub, deg = false, width = 470, height = 132, us, ff, series, tick }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} preserveAspectRatio="xMidYMid meet" style={{ display: 'block', marginTop: 5 }}>
        <line x1={X(T2000)} y1={padT} x2={X(T2000)} y2={padT + plotH} stroke={CC.ink4} strokeWidth="1" strokeDasharray="1 3" />
        <text x={X(T2000) + 3} y={padT + 8} style={{ fontFamily: MONO, fontSize: 8.5, fill: CC.ink4 }}>~2000</text>
        {scrub ? [
    ...
    <div style={{ background: 'transparent', display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, justifyContent: 'safe center', overflow: 'auto' }}>
      <div style={{ flexShrink: 0, padding: `clamp(28px,4.5vh,52px) 44px 8px ${_PLX}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12 }}>
          ...
          {onSkip && <button onClick={onSkip} style={_skipStyle}>skip to the story →</button>}
        </div>
...
// the closing rail (matches the story's "ended" card) — the comparison + handoff
function PrologueEndRail({ usArr, ffArr, onToStory, onSkip }) {
  return (
    <div style={{ background: 'transparent', display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, justifyContent: 'safe center', overflow: 'auto' }}>
      <div style={{ flexShrink: 0, padding: `clamp(24px,4vh,44px) 44px 8px ${_PLX}` }}>
        <Eyebrow>Prologue · so what gives?</Eyebrow>
        ...
        <p style={{ margin: '12px 0 0', ...PROSE, color: CC.ink2, maxWidth: 470 }}>
          But the real <strong>split</strong> runs far past anything the engine reaches alone, and keeps climbing after ~2000 while the engine stalls. That gap is what mechanisms can’t make by themselves — <strong>external shocks</strong>: partisan media, dated events, the timing of who mobilized when. The engine supplies the forces; history supplies the shove.
        </p>
        <div style={{ marginTop: 18, maxWidth: 480 }}>
          <PChart title="Party separation — the split" sub="tracks, then peels away after ~2000" us={usArr('sep')} ff={ffArr('sep')} />
          <PChart title="Out-party warmth — the feelings" sub="nearly identical — the engine cools on its own" us={usArr('aff')} ff={ffArr('aff')} deg />
          <CompareLegend />
        </div>
        <div style={{ marginTop: 22, marginBottom: 12, maxWidth: 480 }}>
          <button onClick={onToStory} style={{ width: '100%', padding: '13px 18px', borderRadius: DS.rad.pill, border: 'none', background: CC.ink, color: '#fff', cursor: 'pointer', fontFamily: SANS, fontSize: 14, fontWeight: 500 }}>
...
function ProloguePage({ onToStory, onSkip }) {
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => { e.preventDefault(); setPlaying(false); setTick((t) => Math.max(0, Math.min(LAST, t + e.deltaY * 0.06))); };
    ...
    <>
      <div ref={wrapRef} style={{ flex: 1, minHeight: 0, position: 'relative', overflow: 'hidden', background: CC.bg }}>
        ...
        <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: 'min(54%, 820px)', display: 'flex', flexDirection: 'column', minHeight: 0, zIndex: 3 }}>
          {ended
            ? <PrologueEndRail usArr={usArr} ffArr={ffArr} onToStory={onToStory} onSkip={onSkip} />
            : <PrologueBeatRail beat={beat} tick={tick} year={year} onSkip={onSkip} />}
        </div>
      </div>
