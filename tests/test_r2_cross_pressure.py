...
def _pull_delta(align, damp):
    """PartyPull ideology-path delta magnitude toward the party centroid for an
    agent at distance from the centroid, at a given cross-pressure damp."""
    a = Agent(id=0, state=AgentState(
        ideology=np.array([0.0, 0.0], dtype=float),
        attrs={"party": 0, "identity_alignment": float(align),
               "identity_strength": 0.5, "stubbornness": 0.0}))
    space = ContinuousSpace2D()
    space.rebuild([a])
    env = Environment(attrs={"parties": {0: np.array([0.8, 0.0])}})
    rule = PartyPull(strength=0.1, xpressure_damp=damp)
    d = rule.apply(a, space, env, np.random.default_rng(0)).d_ideology
    return float(np.linalg.norm(d))


def test_partypull_cross_pressure_brake():
    """R2 retarget: a cross-pressured agent (alignment=0 → xp=1) is pulled toward
    the party centroid LESS than a fully-stacked one (alignment=1 → xp=0)."""
    d_cross = _pull_delta(align=0.0, damp=0.6)
    d_stack = _pull_delta(align=1.0, damp=0.6)
    assert d_cross > 0.0 and d_stack > 0.0
    assert d_cross < d_stack
    # factor for align=0,damp=0.6 is 0.4 → exactly 0.4x the undamped pull.
    assert abs(d_cross - 0.4 * d_stack) < 1e-12


def test_partypull_damp_off_alignment_independent():
    """damp 0.0 → identity_alignment has no effect on the pull (bit-identical)."""
    assert abs(_pull_delta(0.0, 0.0) - _pull_delta(1.0, 0.0)) < 1e-12
...
def test_build_off_damps_zero():
    ...
    assert _rule(eng, "PartyPull").xpressure_damp == 0.0
...
def test_build_on_passes_damps():
    ...
    assert _rule(eng, "PartyPull").xpressure_damp == 0.5
